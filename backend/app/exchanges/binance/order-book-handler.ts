...
const debug = Debug('order-book-handler-binance')
...
let init = function (snapshot: i.ISnapshotInt) {
  ...
  orderBookCache.forEach(orderBookCacheItem => {
    const u = orderBookCacheItem.sequenceEnd;
    const asks = orderBookCacheItem.changes.asks;
    if (asks.length > 0) {
      orderBook.asks = updateHelper(orderBook.asks, asks, u, gi.EOrder.Asc);
    }
    const bids = orderBookCacheItem.changes.bids;
    if (bids.length > 0) {
      orderBook.bids = updateHelper(orderBook.bids, bids, u, gi.EOrder.Desc);
    }
  })
...
let updateHelper = function (originalArray: Array<[Big, Big]>, changesArray: Array<[Big, Big]>, u: number, order: gi.EOrder): Array<[Big, Big]> {
  // Add, update or remove
  let updatedArray = originalArray;
  for (const change of changesArray) {
    debug('Handling change', change);
    if (u > orderBook.sequence) {
      if (change[1].eq(0)) {
        // remove
        ...
        if (!updated) {
          ...
          updatedArray.splice(index, 0, change); // we don't need sequence number anymore
...
let update = function (update: i.IL2updateInt) {
  ...
  const u = update.sequenceEnd;
  if (update.changes.bids.length > 0) {
    orderBook.bids = updateHelper(orderBook.bids, update.changes.bids, u, gi.EOrder.Desc);
    debug('Updated orderBook.bids', JSON.stringify(orderBook.bids));
  }

  if (update.changes.asks.length > 0) {
    orderBook.asks = updateHelper(orderBook.asks, update.changes.asks, u, gi.EOrder.Asc);
    debug('Updated orderBook.asks', JSON.stringify(orderBook.asks));
  }

  debug('Updated order book for [' + i.BINANCE + '] old sequence [' + orderBook.sequence + '] new sequence [' + update.sequenceEnd + '] at [' + moment().format() + ']');
